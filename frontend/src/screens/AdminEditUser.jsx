...
import { useAdminUserUpdateMutation, useGetSingleUserMutation } from '../slices/adminApiSlice';
...
function AdminEditUser() {
    ...
    const [updateUser, { isLoading }] = useAdminUserUpdateMutation()
    const [getUser , {isLoadingSingle}] = useGetSingleUserMutation()

    useEffect(() => {
        const fetchUser = async()=>{
            try {
                const user = await getUser(id).unwrap('')
                setUserInfo(user)
            } catch (error) {
    ...
    useEffect(() => {
        // This useEffect will run whenever 'userInfo' changes
        if (userInfo) {
    ...
    const submitHandler = async (e) => {
        ...
        if (name.length < 3) {
            ...
            try {
                const res = await updateUser({id,name,email,password}).unwrap("");
        ...
        <main className='d-flex justify-content-center align-items-center' style={{ height: "90vh" }}>
            <div className='p-3 border border-2' style={{ width: '20rem' }}>
                <form onSubmit={submitHandler} encType="multipart/form-data">
                    <h4 className='text-center fw-bold mb-3'>UPDATE USER</h4>
